 ...
 */
public class ChrWebsocketStarter {

    private WsServerStarter wsServerStarter;
    private ServerGroupContext serverGroupContext;

    public ChrWebsocketStarter(int port, ChrWsMsgHandler wsMsgHandler) throws Exception {

        wsServerStarter = new WsServerStarter(port, wsMsgHandler);
        serverGroupContext = wsServerStarter.getServerGroupContext();
        serverGroupContext.setName(ChrServerConfig.PROTOCOL_NAME);
        serverGroupContext.setServerAioListener(ChrWsServerAioListener.chr);

        //设置ip监控
        serverGroupContext.setIpStatListener(ChrStatListener.chr);
        //设置ip统计时间段
        serverGroupContext.ipStats.addDurations(ChrServerConfig.IpStatDuration.IPSTAT_DURATIONS);
        //设置心跳超时时间
        serverGroupContext.setHeartbeatTimeout(ChrServerConfig.HEARTBEAT_TIMEOUT);
...
//            String keyStorePwd = "214323428310224";
    ...
    public static void start() throws Exception {
        ChrWebsocketStarter appStarter = new ChrWebsocketStarter(ChrServerConfig.SERVER_PORT, ChrWsMsgHandler.chr);
